...
uint8_t *FindSectionHeader(uint8_t *elf, size_t elf_size, const char *name,
                           int *out_idx = nullptr);
[[nodiscard]] bool AddTrampolineSymbols(MallocBuffer &elf_buf,
                                        const std::vector<Trampoline> &trampolines,
...
Trampoline BuildTrampoline(const std::vector<std::string> &asm_lines,
                           ...
                           const std::string &cpu, const LLVMState &llvm_state);
[[nodiscard]] bool MatchRule(const RewriteRule &rule,
...
amd_comgr_status_t RetargetCodeObjectB0A0Grow(const void *elf_data,
                                               size_t elf_size,
                                               void **out_data,
                                               size_t *out_size,
...
amd_comgr_status_t TranspileCodeObject(const void *elf_data, size_t elf_size,
